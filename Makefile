...
project-init: ## Init project First
	U_ID=${UID} docker-compose up -d && docker exec -it ${DOCKER_APP} composer install && cp .env.example .env && php artisan key:generate && php artisan migrate && php vendor/bin/phpunit
